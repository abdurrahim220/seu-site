...
const AllAndroidCourse = () => {
  const [androids,setLoading] = useAndroidFetch()
  ...
  const handleDelete = (data) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch(`http://localhost:5000/android/${data._id}`, {
                method: 'DELETE',
            })
                .then(res => res.json())
                .then(data => {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-4 lg:gap-8 py-10">
        <div className="h-auto rounded-lg lg:col-span-3">
          <div className='grid grid-cols-1 mx-auto justify-center gap-3 md:grid-cols-3 justify-items-center pt-5 px-3'>
            {
              androids.map(data => {
                return <ActionCard key={data._id} handleDelete={handleDelete} data={data} />
              })
            }
          </div>
        ...
        <div className="h-32 lg:pt-6 rounded-lg">
          <div className="card w-80  lg:w-90 bg-base-100">
...
export default AllAndroidCourse